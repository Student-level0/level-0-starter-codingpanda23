...
public class TurtleTreasureHunt implements KeyEventDispatcher {
	...
	private void spaceBarWasPressed() {
		...
		// make a pop-up to tell the Tortoise where to go next
		if (tortoiseLocationX == 510 && tortoiseLocationY == 300) {
			JOptionPane.showMessageDialog(null, "A fish from a movie has something to say, reversed.");
		...
		if (tortoiseLocationX == 330 && tortoiseLocationY == 410) {
			JOptionPane.showMessageDialog(null, "wolloh eson");
		}
		if (tortoiseLocationX == 180 && tortoiseLocationY == 80) {
			JOptionPane.showMessageDialog(null, "Congatulations! You've got the key! Now for your reward...");
			Tortoise.explode();
		}
